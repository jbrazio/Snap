...
SpriteMorph.prototype.enableSpringTo = function(name, stiffness, length) {
    var p = new Process();
    var morphs = p.getObjectsNamed(name, this);

    if (morphs.length != 1) {
        return;
    }
    var other = morphs[0];
    console.log("Adding spring to " + other.name);
    if (this.rigidBody && this.rigidBody instanceof RigidBody) {
        if (!other.rigidBody || other.rigidBody instanceof RigidBody) {
            other.rigidBody = new RigidBody(other, 0, 0);
        }
        this.rigidBody.addSpring(other.rigidBody, stiffness, length);
    }
}

SpriteMorph.prototype.disableSpringTo = function(other) {
    if (this.rigidBody && this.rigidBody instanceof RigidBody) {
        if (other.rigidBody && other.rigidBody instanceof RigidBody) {
            this.rigidBody.removeSpring(other.rigidBody);
        }
    }
}
